document.addEventListener("DOMContentLoaded", () => {
    ...
    function checkAuth() {
        ...
        if (token) {
            ...
            loadSensors();
            if (!updateInterval) {
                updateInterval = setInterval(() => loadSensors(true), 3000); // Polling a cada 3s
            }
        } else {
            showView(loginView);
            if (updateInterval) {
    ...
    async function loadSensors(silent = false) {
        ...
        try {
            const user = JSON.parse(localStorage.getItem("user") || "{}");
            const response = await fetch(`/api/sensors?clienteEmail=${encodeURIComponent(user.email)}`, {
                headers: {
            ...
            if (sensors.length === 0) {
                sensorList.innerHTML = "<p>Você não possui nenhum sensor cadastrado.</p>";
                return;
            }

            // Atualização silenciosa apenas dos valores para não piscar a tela
            if (silent && document.querySelectorAll('.sensor-widget').length === sensors.length) {
                sensors.forEach(sensor => {
                    const valElement = document.getElementById(`val-${sensor._id}`);
                    if (valElement) {
                        valElement.textContent = sensor.valor !== null ? sensor.valor : '--';
                    }
                });
                return;
            }
    ...
    window.editSensor = async (id, currentName) => {
        const novoApelido = prompt("Digite o novo apelido para o sensor:", currentName);
